...
ブラウザで `http://127.0.0.1:3000` を開くと、provisioning 済みの
`Kuroshio Observability Tutorial` dashboard をすぐ使えます。

ここでは少なくとも次を最初から見られます。

- SMTP connections
- worker delivery success
- spool store metrics
- domain throttle metrics
- Loki の `kuroshio` logs

trace を詳しく見るときは、左メニューから `Explore` を開きます。
...
Grafana 上で metrics を直接見たい場合は、datasource に `Prometheus` も provisioning 済みです。
